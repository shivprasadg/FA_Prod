...
Begin Report
    ...
    RecordSource ="SELECT PD.LocationName, PD.LState, PD.SchNo, PD.sttaxtype, PD.UnitTaxExempt, Sum"
        ...
        "ineTotal) AS LineTotal, Sum(PD.PDDAYS) AS TotPDDAYS FROM vw_PerDiemInvoice AS PD"
        " GROUP BY PD.LocationName, PD.LState, PD.SchNo, PD.sttaxtype, PD.UnitTaxExempt, "
        "PD.BillID, PD.SGrpID HAVING (((PD.BillID)=[Forms]![Billing]![BillID]) AND ((PD.S"
        "GrpID)=[Forms]![Billing]![GroupID]));"

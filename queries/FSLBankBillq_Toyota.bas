Operation =1
Option =0
Having ="(((Units.UnitStatus)=\"A\" Or (Units.UnitStatus)=\"TR\") And ((Bills.BillID)=For"
    "ms!Billing!BillingSubForm.Form!BillID) And ((Locations_2.LocObsolete)=No) And (("
    "Locations_2.LocationType)=\"billing\") And ((SchGrp.BLCD) Is Not Null) And ((Uni"
    "ts.unitoffleasedt) Is Null Or (Units.unitoffleasedt)>=[Billfrom]))"
Begin InputTables
    Name ="Schedule"
    Name ="PDstdtq"
    Name ="Locations"
...
Begin OutputColumns
    ...
    Alias ="TaxAmt"
    Expression ="IIf([unittaxexempt]=True,0,CCur(Round([locations].[LTaxRate]*[unitrent],2)))"
    Alias ="LineTot"
    Expression ="IIf([unittaxexempt]=True,0,CCur(Round([locations].[LTaxRate]*[unitrent],2)))+[un"
        "itrent]"

...
Begin Form
    ...
    RecSrcDt = Begin
        0xcc3b9c6576f8e440
    End
    RecordSource ="SELECT TaxStates.StateName, TaxStatesBase.*, TaxType.TaxFullName, TaxType.TaxAbb"
        "reviation, TaxType.TaxDescription, IIf(Not IsNull([StartsOn]) And IsNull([EndsOn"
        "]),-1,0) AS IsActive, TaxType.TaxFullName FROM (TaxStatesBase INNER JOIN TaxStat"
        "es ON TaxStatesBase.TaxStatesID = TaxStates.Id) LEFT JOIN TaxType ON TaxStatesBa"
        "se.TaxTypeId = TaxType.Id ORDER BY TaxStates.StateName, IIf(Not IsNull([StartsOn"
        "]) And IsNull([EndsOn]),-1,0) DESC , TaxType.TaxFullName DESC;"
